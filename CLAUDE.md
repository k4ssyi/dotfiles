...
| `.claude/rules/` | 条件付きルール（paths指定で対象ファイル限定） |
...
| `ghostty/` | Ghostty設定 |
| `cmux/` | cmux設定 (`~/.config/cmux/cmux.json` にシンボリックリンク。tmux互換キーバインド) |
| `tmux/` | tmux設定 |
...
`scripts/` 配下のインストールスクリプトは macOS 標準の `/bin/bash` (3.2) で実行される。
Homebrew の bash 5.x をインストールするのはスクリプト自身のため、以下の機能は使用禁止:

- `declare -A`（連想配列）→ 並列配列で代替
- `${var,,}`（小文字変換）→ `tr '[:upper:]' '[:lower:]'` で代替
- `readarray` / `mapfile` → `while read` ループで代替

### GNU/BSD コマンド互換

`zsh/.zprofile` で `coreutils`, `grep`, `make` の gnubin を PATH に追加しているため、
対話シェルでは GNU 版が優先される。スクリプト内で BSD 固有オプションを使う場合はフォールバックを書くこと:

```bash
# 良い例: POSIX 互換（GNU/BSD 両方で動作）
available_kb=$(df -k . | tail -1 | awk '{print $4}')

# 良い例: GNU → BSD フォールバック
result=$(grep --color=auto pattern 2>/dev/null || grep pattern)
